...
TJBot.prototype.defaultConfiguration = {
    ...
    listen: {
        microphoneDeviceId: "plughw:1,1", // plugged-in USB card 1, device 0; see arecord -l for a list of recording devices
...
TJBot.prototype._createServiceAPI = function(service, credentials) {
    ...
    switch (service) {
        ...
        case 'speech_to_text':
            ...
            //     version: 'v1'
            // });
            var SpeechToTechV1 = require("@google-cloud/speech");
            this._stt = new SpeechToTechV1.SpeechClient({
                projectId : 'tjbotspeechtotext-16730',
...
TJBot.prototype.listen = function(callback) {
    ...
    this._micRecognizeStream = this._stt.streamingRecognize(request);
	...
	//console.log('transcript : ' + transcript);
        //winston.info("TJBot heard: " + JSON.stringify(transcript));
	if(transcript.results.length === 0){
	   winston.info('Error : Audio data is being streamed too fast..!!');
	} else {
	   winston.info('TJBot heard:' + JSON.stringify(transcript.results[0].alternatives[0].transcript));
	}
